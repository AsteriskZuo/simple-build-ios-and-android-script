...
echo "# Build ios curl shell script.                                                #" >/dev/null
echo "#                                                                             #" >/dev/null
...
function ios_curl_printf_variable() {
...
function ios_curl_pre_tool_check() {
...
function ios_curl_build_config_make() {
    ...
    if [[ "${library_arch}" == "x86-64" ]]; then
        ...
        ./Configure --host=$(ios_get_build_host "${library_arch}") --prefix="${library_arch_path}" --disable-shared --enable-static --enable-ipv6 --without-libidn2 --with-ssl="${openssl_output_dir}/${library_arch}" --with-nghttp2="${nghttp2_output_dir}/${library_arch}" >"${library_arch_path}/log/output.log" 2>&1 || common_die "configure error!"

    elif [[ "${library_arch}" == "armv7" ]]; then

        ./Configure --host=$(ios_get_build_host "${library_arch}") --prefix="${library_arch_path}" --disable-shared --enable-static --enable-ipv6 --with-ssl="${openssl_output_dir}/${library_arch}" --with-nghttp2="${nghttp2_output_dir}/${library_arch}" >"${library_arch_path}/log/output.log" 2>&1 || common_die "configure error!"
...
function ios_curl_archive() {
    local library_id=$1
    local static_library_list=()
    for ((i = 0; i < ${#IOS_ARCHS[@]}; i++)); do
        local static_library_file_path="${curl_output_dir}/${IOS_ARCHS[i]}/lib/lib${COMMON_LIBRARY_NAME}.a"
    ...
    if [ 0 -lt ${#static_library_list[@]} ]; then
        util_remove_dir "${curl_output_dir}/lipo"
        util_create_dir "${curl_output_dir}/lipo"
        lipo ${static_library_list[@]} -create -output "${curl_output_dir}/lipo/lib${COMMON_LIBRARY_NAME}-universal.a"
    fi
}
